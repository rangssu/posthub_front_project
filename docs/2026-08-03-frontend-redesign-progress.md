...
코드 작업은 끝났다. 자동 검증(테스트 100개 / `build` / lint)은 전부 통과하지만 **사람 눈이
필요한 항목**이 남아 있다. `npm run dev` 후 **`localhost:5173`**으로 연다(dev 서버가 IPv6에만
바인딩돼 `127.0.0.1:5173`은 붙지 않는다).

- [ ] 7개 페이지 × 라이트/다크 — 읽을 수 없는 색 조합이 없는지
- [ ] 폭 360px — 게시글 목록이 메타 한 줄로 접히는지, 가로 스크롤이 없는지
- [ ] **`PostTable`의 제목 말줄임** — `table-layout: fixed` 없이 `max-w-0` + `truncate`로만
      자르고 있다. jsdom으로는 확인할 수 없어 계속 미확인으로 남아 있는 항목이다.
      안 걸리면 `<table>`에 `table-fixed`를 주는 것이 정석이다.
- [ ] 게시판 생성·수정·삭제 다이얼로그
- [ ] 키보드만으로: 삭제 → Enter → Tab 순환 → Escape → 포커스가 삭제 버튼으로 복귀
- [ ] 다크모드로 두고 새로고침 — 흰 화면이 번쩍이지 않는지
- [ ] 잘못된 이메일로 가입 시도 — 브라우저 기본 말풍선이 아니라 우리 인라인 오류가 뜨는지
      (`noValidate`가 실제 브라우저에서 먹는지 확인하는 자리다)

그다음 `finishing-a-development-branch`.
